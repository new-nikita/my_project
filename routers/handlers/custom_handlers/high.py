...
highprice_router = Router()


@highprice_router.message(Command("high"))
async def high(msg: Message, state: FSMContext):
...
@highprice_router.message(MyState.count_in_button)
async def count_button_user(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        text="Нашел наушники по убываю цены.",
    ...
    for elem in sorted_low:
        ...
        await msg.answer(
            text='Бренд: {}\nЦвет: {}\nЦена: {}\nЦена со скидкой: {}\nРейтинг: {}\nКоличесво отзывов: {}\nid: {}'.format(
                elem.get('brand', None), elem.get('colors', None), elem.get('priceU', None),
